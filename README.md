...
Apply `supabase/migrations/023_email_alerts.sql`, `024_email_alert_options.sql`, and `025_email_alert_occurrences.sql` in the Supabase SQL Editor, then configure `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `SUPABASE_SERVICE_ROLE_KEY`, and `CRON_SECRET` in Vercel. The sending address must be verified in Resend. The scheduled sender runs through `/api/cron/send-alerts`. Repeating occurrences run at the configured three-hour slots and require a Vercel plan that supports multiple daily cron runs.