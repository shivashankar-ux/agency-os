...
Apply `supabase/migrations/023_email_alerts.sql`, `024_email_alert_options.sql`, `025_email_alert_occurrences.sql`, and `026_email_alert_clients.sql` in the Supabase SQL Editor, then configure `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `SUPABASE_SERVICE_ROLE_KEY`, and `CRON_SECRET` in Vercel. The sending address must be verified in Resend. The scheduled sender runs through `/api/cron/send-alerts`. Repeating occurrences require a scheduler that can run at the selected interval; Vercel Hobby is configured for a daily check, while multiple daily occurrence slots require Vercel Pro or an external scheduler.